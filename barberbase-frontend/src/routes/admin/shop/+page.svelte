...
<div class="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900">
	<div class="max-w-2xl mx-auto p-6">
		...
		{/if}
		{#if form?.success}
			<div class="bg-green-900/30 border border-green-700 rounded-xl p-4 mb-6 text-green-400 text-sm">✓ Shop status updated</div>
		{/if}
		...
		{#if data.shopStatus}
			<div class="bg-slate-800 border border-slate-700 rounded-2xl p-6 mb-6 shadow-xl">
				<div class="flex items-center justify-between mb-4">
					...
					<span class="px-3 py-1 rounded-full border text-sm font-medium {statusBadge(data.shopStatus.shop_status)}">
				...
				<div class="grid grid-cols-2 gap-4 text-sm">
					<div>
						...
						<p class="text-white">{data.shopStatus.manual_override_active ? '✓ Active' : '— None'}</p>
					</div>
					{#if data.shopStatus.override_expires_at}
						<div>
							<p class="text-slate-400 text-xs mb-1">Expires At</p>
							<p class="text-white text-xs">{new Date(data.shopStatus.override_expires_at).toLocaleString('en-IN')}</p>
						</div>
					{/if}
				</div>
				...
				{#if data.shopStatus.arrival_pin}
					<div class="mt-4 p-4 bg-amber-900/20 border border-amber-700/50 rounded-xl">
						<p class="text-xs text-amber-400 font-semibold mb-1">Counter PIN — show this to customers for arrival verification</p>
						<p id="arrival-pin-display" class="text-4xl font-bold font-mono text-amber-300 tracking-widest">{data.shopStatus.arrival_pin}</p>
						<form method="POST" action="?/regeneratePin" use:enhance class="mt-3">
							<button type="submit" class="text-xs text-amber-500 hover:text-amber-300 transition-colors">
								↻ Regenerate PIN
							</button>
						</form>
					</div>
				...
				{#if form?.pin_success && form?.new_pin}
					<div class="mt-3 p-3 bg-green-900/20 border border-green-700 rounded-xl">
						...
						<p class="text-2xl font-bold font-mono text-green-300 tracking-widest">{form.new_pin}</p>
					</div>
				{/if}
			</div>
		{/if}
		...
		<div class="bg-slate-800 border border-slate-700 rounded-2xl p-6 shadow-xl">
			...
			<form id="set-shop-status-form" method="POST" action="?/setStatus" use:enhance>
					...
					{#each ['open', 'closed', 'temporarily_closed'] as s}
						<label class="cursor-pointer">
							<input
								...
								class="sr-only peer"
							/>
							<div class="border rounded-xl p-3 text-center text-sm font-medium transition-all border-slate-600 text-slate-400 peer-checked:border-amber-500 peer-checked:text-amber-400 peer-checked:bg-amber-900/20 hover:border-slate-500 hover:text-white">
				...
				{#if selectedStatus === 'temporarily_closed'}
					<div class="mb-4">
						<label for="expires-select" class="block text-xs text-slate-400 mb-2">Close for how long?</label>
						<select id="expires-select" name="expires_in_minutes" bind:value={expiresInMinutes} class="w-full bg-slate-700 border border-slate-600 rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:ring-2 focus:ring-amber-500">
...
{#if showModal}
	<div class="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 p-4">
		<div id="shop-status-conflict-modal" class="bg-slate-800 border border-slate-700 rounded-2xl p-6 max-w-md w-full shadow-2xl">
			<h3 class="text-xl font-bold text-white mb-2">⚠️ Active Customers Waiting</h3>
			<p class="text-slate-300 mb-6 text-sm">
				There {activeEntryCount === 1 ? 'is' : 'are'} <strong class="text-white">{activeEntryCount}</strong> customer{activeEntryCount !== 1 ? 's' : ''} waiting. What would you like to do?
			</p>
			<div class="grid grid-cols-1 gap-3">
				<!-- Serve them first -->
				<form method="POST" action="?/setStatus" use:enhance>
					<input type="hidden" name="status" value={pendingStatus} />
					{#if pendingExpires !== null}<input type="hidden" name="expires_in_minutes" value={pendingExpires} />{/if}
					<input type="hidden" name="modal_action" value="finish_remaining" />
					<button id="modal-finish-remaining-btn" type="submit" class="w-full bg-blue-600 hover:bg-blue-500 text-white font-bold py-3 rounded-xl transition-all text-sm">
				...
				<form method="POST" action="?/setStatus" use:enhance>
					...
					<button id="modal-expire-remaining-btn" type="submit" class="w-full bg-red-700 hover:bg-red-600 text-white font-bold py-3 rounded-xl transition-all text-sm">
