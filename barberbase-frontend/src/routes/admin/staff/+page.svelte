	...
	<div class="max-w-3xl mx-auto p-6">
		...
		{#if data.staffMembers.length === 0}
			<div class="bg-slate-800 border border-slate-700 rounded-2xl p-12 text-center">
				<p class="text-slate-400 text-lg mb-2">No staff members yet</p>
				<p class="text-slate-500 text-sm">Click "+ Add Staff" to add your first team member.</p>
			</div>
		{:else}
					...
					<tbody class="divide-y divide-slate-700/50">
						{#each data.staffMembers as member}
							<tr class="hover:bg-slate-700/20 transition-colors">
								...
								<td class="px-4 py-4 text-sm text-slate-300">{roleLabel(member.role)}</td>
								<td class="px-4 py-4 text-sm text-slate-300">{statusLabel(member.status)}</td>
							</tr>
						{/each}
					</tbody>
				</table>
