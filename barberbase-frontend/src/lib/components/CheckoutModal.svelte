<script lang="ts">
	...
	const paymentLinesPaise = $derived(
		paymentLines.map((line) => ({
			...
			provider_reference_id: line.method === 'upi' && line.provider_reference_id ? line.provider_reference_id : null
		}))
	);
...
<div class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm" role="dialog" aria-modal="true">
	<div class="w-full max-w-lg bg-slate-900 border border-slate-800 rounded-2xl shadow-2xl overflow-hidden text-slate-100 flex flex-col max-h-[90vh]">
		<!-- Modal Header -->
		<div class="px-6 py-4 border-b border-slate-800 flex justify-between items-center bg-slate-950">
			<div>
				...
				<p class="text-xs text-slate-400">Token #{entry.token_number} — {entry.customer?.name || 'Walk-in Customer'}</p>
			</div>
			<button
				type="button"
		...
		<form onsubmit={handleSubmit} class="flex-1 overflow-y-auto p-6 space-y-6">
			...
			<div>
				<h3 class="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-2">Rendered Services</h3>
					...
					{#each entry.services as service}
						<div class="px-4 py-3 flex justify-between text-sm">
							...
							<div class="text-right">
								<div class="font-bold text-slate-100">₹{(service.price_paise / 100).toFixed(2)}</div>
			...
			<div class="space-y-3">
				...
				<div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
					<div>
						<label for="discount-amt" class="block text-xs font-medium text-slate-400 mb-1">Discount Amount (₹)</label>
						<input
							type="number"
							id="discount-amt"
							step="0.01"
					...
					<div>
						<label for="discount-reason" class="block text-xs font-medium text-slate-400 mb-1">Discount Reason</label>
						<input
							type="text"
							id="discount-reason"
			...
			<div class="space-y-3">
				<div class="flex justify-between items-center">
					<h3 class="text-xs font-semibold text-slate-400 uppercase tracking-wider">Split Payment Lines</h3>
					<button
						type="button"
					...
					{#each paymentLines as line, idx}
						<div class="bg-slate-950 border border-slate-800 rounded-xl p-3 space-y-2 relative">
							...
							<div class="grid grid-cols-2 gap-2 pr-6">
								<div>
									<label for="payment-method-{idx}" class="block text-[10px] font-medium text-slate-500 mb-1">Method</label>
									<select
								...
								<div>
									<label for="payment-amount-{idx}" class="block text-[10px] font-medium text-slate-500 mb-1">Amount (₹)</label>
									<input
										type="number"
							...
							{#if line.method === 'upi'}
								<div class="pt-1">
									<label for="payment-upi-ref-{idx}" class="block text-[10px] font-medium text-slate-500 mb-1">UPI Provider Ref ID (Optional)</label>
									<input
										type="text"
			...
			<div class="bg-slate-950 border border-slate-800 rounded-xl p-4 space-y-2 text-sm">
				...
				{/if}
				<div class="border-t border-slate-800 pt-2 flex justify-between font-semibold text-slate-100">
			...
			{#if errorMessage || (mounted && isMismatch)}
				<div class="bg-red-950/40 border border-red-900/50 rounded-xl p-3 text-xs text-red-400 flex items-start space-x-2">
					...
					<div>{errorMessage || 'Payment mismatch: Entered payment lines must sum exactly to expected total.'}</div>
				</div>
			{/if}
